    ...
    * */
    ...
    public void setUniforms(float[] matrix, int[] textureId) {
        ...
        for (int i = 0; i < MAP_DIVISION_SIZE; i++) {
            ...
            //GLES30.setVertexAttribPointer(0, aPositionLocation, POSITION_COMPONENT_COUNT, 0);
            vertexFloatBuffer.position(8 * i);
            GLES30.glEnableVertexAttribArray(vPositionLocation);
            GLES30.glVertexAttribPointer(vPositionLocation, 2,
                    GLES30.GL_FLOAT, false, 8, vertexFloatBuffer);
            // UV
            colorVertexFloatBuffer.position(8 * i);
            GLES30.glEnableVertexAttribArray(a0TexCoordLocation);
            GLES30.glVertexAttribPointer(a0TexCoordLocation, 2, GLES30.GL_FLOAT, false, 0,
                    colorVertexFloatBuffer);
            GLES30.glDrawElements(GLES30.GL_TRIANGLES, iIndexArray.length, GLES30.GL_UNSIGNED_BYTE, indexShortBuffer);
            GLES30.glBindTexture(GLES30.GL_TEXTURE_2D, 0);
        }
        GLES30.glDisable(GLES30.GL_CULL_FACE);
        GLES30.glFlush();
